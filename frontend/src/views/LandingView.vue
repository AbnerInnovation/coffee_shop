<template>
  <section class="max-w-6xl mx-auto px-6 py-16">
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-50 to-white dark:from-slate-900 dark:to-slate-900 ring-1 ring-inset ring-indigo-100 dark:ring-indigo-900/40 mb-14">
      <div class="pointer-events-none absolute -top-10 -right-10 h-40 w-40 bg-indigo-200/40 dark:bg-indigo-900/30 rounded-full blur-3xl"></div>
      <div class="pointer-events-none absolute -bottom-10 -left-10 h-40 w-40 bg-sky-200/40 dark:bg-sky-900/30 rounded-full blur-3xl"></div>
      <header class="text-center px-6 py-12 md:py-16">
        ...
        <p class="mt-4 text-gray-600 dark:text-slate-300 max-w-2xl mx-auto">
          Optimiza mesas, pedidos, cocina y caja en minutos. Escalable de taquerías a cadenas. Añade módulos cuando los necesites.
        </p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
      <div v-for="addon in addons" :key="addon.title" class="p-6 border rounded-2xl bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 hover:shadow-md transition">
        ...
        <ul v-if="addon.items" class="mt-3 text-sm space-y-1">
          <li v-for="(item, idx) in addon.items" :key="idx" class="text-slate-700 dark:text-slate-300">• {{ item }}</li>
        </ul>
        <div v-else class="mt-4 text-lg font-extrabold text-slate-900 dark:text-slate-100">{{ addon.price }}</div>
      </div>
    </div>
